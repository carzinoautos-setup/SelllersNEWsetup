  ...
  const toggleOptionWithAlternate = (
    ...
    setSelectedOptions((prev) => {
      ...
      if (hasPrimary || hasAlternate) {
        return prev.filter((p) => p !== primaryKey && p !== alternateKey);
      }
          ...
          <section className="w-full mb-8">
            <div className="max-w-[1540px] mx-auto">
              <div className="flex flex-col gap-5 rounded-2xl border border-[#EDEDED] bg-white p-[30px]">
                <div className="w-full max-w-[952px] rounded-2xl bg-white py-6 flex flex-col items-center justify-center relative mx-auto">
                  <div className="flex flex-col items-center gap-4 w-full max-w-[220px]">
                    {/* Select Photos Button */}
                    <button
                      type="button"
                      onClick={() =>
                        document.getElementById("upload-input-addlisting")?.click()
                      }
