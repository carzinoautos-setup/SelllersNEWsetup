          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 items-stretch">
            ...
            <div className="flex items-start">
              <div className="bg-white rounded-2xl p-6 sm:p-8 shadow-lg w-full max-w-[640px] min-h-[320px]">
                <h1 className="text-[26px] sm:text-[30px] font-bold text-[#24272C] font-albert mb-4">Enter your info</h1>
                ...
                <div className="flex items-start gap-3 mb-4">
                  ...
                  <div className="flex-1">
                    <button type="button" onClick={() => setActiveTab("vin")} className={`w-full h-12 rounded-[14px] ${activeTab === "vin" ? "bg-[#24272C] text-white" : "border border-[#24272C] text-[#24272C]"}`}>
                ...
                {flowState === "enter" && (
                  <div className="flex flex-col gap-3">
                    {activeTab === "license" ? (
                      <>
                        <div className="flex h-12 px-3 items-center border border-[#EDEDED] rounded-[14px] bg-white">
                          <input type="text" aria-label="license plate" value={licensePlate} onChange={(e) => setLicensePlate(e.target.value)} placeholder="Enter License Plate #" className="flex-1 text-sm text-[#24272C] bg-transparent outline-none font-inter" />
                        </div>
                        <div className="relative">
                          <select value={selectedState} onChange={(e) => setSelectedState(e.target.value)} className="h-12 w-full px-3 border border-[#EDEDED] rounded-[14px] bg-white text-sm text-[#24272C] font-inter outline-none appearance-none">
                      ...
                      <>
                        <div className="flex h-12 px-3 items-center border border-[#EDEDED] rounded-[14px] bg-white">
                          <input type="text" aria-label="vin" value={vin} onChange={(e) => setVin(e.target.value)} placeholder="Enter Vin #" className="flex-1 text-sm text-[#24272C] bg-transparent outline-none font-inter" />
                        </div>
                        <div className="relative">
                          <select value={vinYear} onChange={(e) => setVinYear(e.target.value)} className="h-12 w-full px-3 border border-[#EDEDED] rounded-[14px] bg-white text-sm text-[#24272C] font-inter outline-none appearance-none">
                    ...
                    <button type="button" onClick={handleGetDetails} className="h-12 w-full rounded-[14px] bg-[#E82121] text-white">Get vehicle details</button>
                  </div>
                )}
            ...
            <div className="flex items-stretch">
              <div className="w-full h-full rounded-2xl overflow-hidden bg-[#F3F4F6] flex items-center justify-center">
                <img src="https://cdn.builder.io/api/v1/image/assets%2F4d1f1909a98e4ebc8068632229306ce4%2F23b891c966f7452fa3f3f303b9253c6a?format=webp&width=1200" alt="vehicle" className="w-full h-full object-cover" />
              </div>
            </div>
          </div>
